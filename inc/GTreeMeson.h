...
public:
    ...
    const   TLorentzVector& SubChargedPi(const Int_t meson, const Int_t subParticle) const {return *((TLorentzVector*)(((TClonesArray*)subChargedPi->At(meson))->At(subParticle)));}
            TLorentzVector& SubParticles(const Int_t meson, const Int_t subParticle)       {if(subParticle>=nSubPhotons[meson]) return SubChargedPi(meson, subParticle); return SubPhotons(meson, subParticle);}
    const   TLorentzVector& SubParticles(const Int_t meson, const Int_t subParticle) const {if(subParticle>=nSubPhotons[meson]) return SubChargedPi(meson, subParticle); return SubPhotons(meson, subParticle);}
};
