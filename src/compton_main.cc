...
int main(int argc, char *argv[])
{
    ...
    PCompton* pcomp = new PCompton;

    // If unset, scan server or config file for file settings
    if(dir_in.length() == 0)
    {
        flag = pcomp->ReadConfig("Input-Directory",0,(Char_t*)serverfile.c_str());
    ...
    if(dir_out.length() == 0)
    {
        flag = pcomp->ReadConfig("Output-Directory",0,(Char_t*)serverfile.c_str());
        flag.erase(0,flag.find_first_not_of(" "));
    ...
    if(file_in.length() == 0)
    {
        flag = pcomp->ReadConfig("Input-File",0,(Char_t*)serverfile.c_str());
        flag.erase(0,flag.find_first_not_of(" "));
    ...
    {
        flag = pcomp->ReadConfig("Output-File",0,(Char_t*)serverfile.c_str());
        flag.erase(0,flag.find_first_not_of(" "));
    ...
    if(pre_in.length() == 0)
    {
        flag = pcomp->ReadConfig("Input-Prefix",0,(Char_t*)serverfile.c_str());
        flag.erase(0,flag.find_first_not_of(" "));
    ...
    if(pre_out.length() == 0)
    {
        flag = pcomp->ReadConfig("Output-Prefix",0,(Char_t*)serverfile.c_str());
        flag.erase(0,flag.find_first_not_of(" "));
    ...
    else { pre_out = "Compton"; cout << "Output prefix:    '" << pre_out << "' chosen by default" << endl; }
    cout << endl;

    // Perform full initialisation
    if(!pcomp->Init(configfile.c_str()))
    {
        cout << "ERROR: PCompton (Collicott) Init failed!" << endl;
        return 0;
    }
    ...
    if(file_in.length() > 0)
    {
        ...
        if(length >= 5)
        {
            ...
            if(!pcomp->StartFile(file_in.c_str(), file_out.c_str())) cout << "ERROR: PCompton (Collicott) failed on file " << file_in << "!" << endl;
            files_found++;
        }
    }
        ...
        while((sys_file=(TSystemFile*)file_iter()))
        {
            ...
            if(length >= 5)
            {
                ...
                if(((strcmp(prefix.c_str(),pre_in.c_str()) == 0)) && (strcmp(suffix.c_str(),".root") == 0))
                {
                    ...
                    files_found++;
                    // Run PCompton
                    if(!pcomp->StartFile(file_in.c_str(), file_out.c_str()))
                        cout << "ERROR: PCompton (Collicott) failed on file " << file_in << "!" << endl;
