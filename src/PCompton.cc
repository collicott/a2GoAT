...
Bool_t	PCompton::Init(const char* configfile)
{
	...
	Double_t Prompt_low 	=  -30;
	Double_t Prompt_high 	=   30;
...
void	PCompton::Reconstruct()
{
	...
	if(GoATTree_GetNParticles() == 2) 
	{
		...
		// Check that both particles pass some rudimentary time cut
		if((GoATTree_GetTime(0) < -20.0) || (GoATTree_GetTime(0) > 20.0)) compton = kFALSE;
		if((GoATTree_GetTime(1) < -20.0) || (GoATTree_GetTime(1) > 20.0)) compton = kFALSE;
...
void  PCompton::PostReconstruction()
{
		...
		for (Int_t j = 0; j < nentries; j ++)
		{
			...
			if(OA[j] <= OACut)
			{
				...
				if (Prompt) 
				{
					...
					if((theta[j] >=  70) && (theta[j] < 90))  phi_prompt_70_90->Fill(phi[j]);								
					if((theta[j] >= 110) && (theta[j] < 130)) phi_prompt_90_110->Fill(phi[j]);
					if((theta[j] >= 135) && (theta[j] < 155)) phi_prompt_135_155->Fill(phi[j]);
				...
				if (Random)
				{
					...
					if(MissMass[j] < 900) continue;
					if(MissMass[j] > 935) continue;	
										
					TC_random->Fill(TC[j]);
					E_random->Fill(E[j]);
					theta_random->Fill(theta[j]);
					if((theta[j] >=  70) && (theta[j] < 90))  phi_random_70_90->Fill(phi[j]);								
					if((theta[j] >= 110) && (theta[j] < 130)) phi_random_90_110->Fill(phi[j]);
					if((theta[j] >= 135) && (theta[j] < 155)) phi_random_135_155->Fill(phi[j]);				
	...
	RandomSubtraction(phi_prompt_70_90,phi_random_70_90, phi_sub_70_90);
	RandomSubtraction(phi_prompt_90_110,phi_random_90_110, phi_sub_90_110);
	RandomSubtraction(phi_prompt_135_155,phi_random_135_155, phi_sub_135_155);
...
void	PCompton::InitHistograms()
{
	...
	phi_prompt_70_90	= new TH1D("phi_prompt_70_90",		"phi_prompt_70_90",		18,-180,180);
	phi_random_70_90	= new TH1D("phi_random_70_90",		"phi_random_70_90",		18,-180,180);
	phi_sub_70_90		= new TH1D("phi_subtracted_70_90",	"phi_subtracted_70_90",	18,-180,180);	

	phi_prompt_70_90->Sumw2(); 
	phi_random_70_90->Sumw2();
	phi_sub_70_90->Sumw2();	
	
	phi_prompt_90_110	= new TH1D("phi_prompt_90_110",		"phi_prompt_90_110",		18,-180,180);
	phi_random_90_110	= new TH1D("phi_random_90_110",		"phi_random_90_110",		18,-180,180);
	phi_sub_90_110		= new TH1D("phi_subtracted_90_110",	"phi_subtracted_90_110",	18,-180,180);	

	phi_prompt_90_110->Sumw2(); 
	phi_random_90_110->Sumw2();
	phi_sub_90_110->Sumw2();	

	phi_prompt_135_155	= new TH1D("phi_prompt_135_155",		"phi_prompt_135_155",		18,-180,180);
	phi_random_135_155	= new TH1D("phi_random_135_155",		"phi_random_135_155",		18,-180,180);
	phi_sub_135_155		= new TH1D("phi_subtracted_135_155",	"phi_subtracted_135_155",	18,-180,180);	

	phi_prompt_135_155->Sumw2(); 
	phi_random_135_155->Sumw2();
	phi_sub_135_155->Sumw2();	
